...
- **`<TerminalGuiVersion>`** (`Directory.Build.props`) — the minimum supported Terminal.Gui, i.e. the NuGet dependency floor. Auto-bumped to the latest **stable** (non-pre-release) Terminal.Gui by `.github/workflows/bump-terminal-gui.yml` (TG's tag-publish dispatch, a fallback schedule, or manually). Develop pre-releases (`*-develop.*`) are ignored by the automatic path — use the workflow's `channel=prerelease` (or `-p:TerminalGuiVersion=<x>`) to canary. The bump is validated with a full build + all test suites, then committed directly to `develop` on green or opened as a PR from `bump/terminal-gui-<version>` on red. A **stable** Editor release requires a **stable** TG pin — `prepare-release.yml` gates on this. Override per-build via `-p:TerminalGuiVersion=<x>`.
